...
from trading.indicators.momentum_indicators import MomentumIndicators, MOMENTUM_INDICATORS
...
def test_wma(sample_data):
    ...
    expected = sample_data.rolling(window=3).apply(lambda x: np.dot(x, weights) / weights.sum(), raw=True)
    pd.testing.assert_series_equal(result, expected)

def test_hma(sample_data):
    ma = MovingAverages(sample_data)
    result = ma.hma(period=3)
    expected = sample_data.rolling(window=3).apply(lambda x: np.sqrt(np.dot(x, x) / 3), raw=True)
    pd.testing.assert_series_equal(result, expected)
