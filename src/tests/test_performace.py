...
def sample_data():
    ...
    data = pd.DataFrame({"Close": [100, 102, 101, 105, 107], "signal": [1, 0, 1, 1, 0]})
    return data
...
def test_calculate_risk(sample_data):
    ...
    expected_risk = (
        perf.data[["returns", "strategy_returns"]].apply(np.exp) - 1
    ).std() * 252**0.5
...
def test_calculate_drawdown(sample_data):
    ...
    pd.testing.assert_series_equal(
        result["drawdown"], expected_drawdown, check_names=False
    )
