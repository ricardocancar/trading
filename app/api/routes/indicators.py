...
async def get_sma(symbol: str, request: Request, short_period: int = 20, long_period: int = 50, span_time:str='1y', interval:str='1d') -> list[dict]:
    ...
    key = f"{span_time}:{interval}:{symbol}:{short_period}:{long_period}"
    ...
    if result is None:
        ...
        await redis_client.set(key,str(data.to_dict(orient="records")))
    else:
    ...
    key = f"{span_time}:{interval}:{symbol}:{short_period}:{long_period}"
    await redis_client.set(key, str(data.reset_index().to_dict(orient="records")))
